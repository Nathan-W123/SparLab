...
/// \brief SIMP minimum-compliance optimisation loop.
///
...
/// The second criterion matters on fine meshes, where a handful of elements can
/// keep oscillating between bounds long after the objective has settled; a
/// design-change-only rule would then never terminate. The result records which
/// criterion fired and the final value of both, so a reader can see whether the
/// design or only the objective had settled. Reaching `max_iterations` without
/// meeting either criterion is reported as non-convergence: the history and the
/// final design are still returned, `converged` is false and a warning names
/// the stalled quantity. Nothing is hidden.
///
...
/// OC is a fixed-point update with a move limit, not a line-search method, and
/// each continuation step raises the penalty, which raises compliance at fixed
/// density. The history records every value so the behaviour can be inspected
